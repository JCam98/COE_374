'''Description: This python module is used to generate a digital map of the 
field using the images captured during the cruise out from the "captureImages.py"
module. In particular, the module contains an image stitching algorithm that 
leverages SIFT feature extraction and FLANN feature matching similar to the 
behavior of the target recognition algorithm. The algorithm performs these operations
on each image in the input image directory and returns an updated stitched map 
each time a new image is stitched onto the current map. This module is designed
to be run manually from the command line after the flight.'''
